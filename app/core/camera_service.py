...
async def start_monitoring_camera(stream_config: StreamConfig) -> Dict[str, Any]:
    """
    ...
    if camera_id in active_streams and active_streams[camera_id]["active"]:
        return {
            "detail": f"Câmera {camera_id} já está sendo monitorada",
            "camera": None,
        }
    ...
    return {
        "detail": f"Iniciado monitoramento para câmera {camera_id}",
        "camera": camera_info,
    }
...
def get_monitored_cameras() -> List[Dict[str, Any]]:
    ...
    for camera_id, camera_data in active_streams.items():
        camera_info = {
            ...
            "stream_config": camera_data["stream_info"]
        }
